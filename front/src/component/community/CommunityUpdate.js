...
const CommunityUpdate = () => {
    ...
    const {communityNo} = useParams(); //communityNo 받아오기
    ...
    //useEffect()
    useEffect(  ()=>{
        console.log('=======게시물 수정====== ',communityNo);
        const data = async() =>{
            ...
            .then((res)=>{
                if(res !==null){
                    ...
                    console.log('useEffect Title Data : ',res.communityTitle)
                    const title={communityTitle:res.communityTitle};
                    setCommunityTitle(title);

                    //응답받은 Content --> block HTML로 변경
                    const blocksFromHTML = convertFromHTML(res.communityContent);
    ...
    const submitCommunity = () =>{
        const data = {
            'communityTitle':communityTitle.communityTitle,
        ...
        <SiteLayout>
            <Form style={{marginTop:'20px'}} >
                <Form.Group className="mb-3" controlId="formBasicEmail">
                    <Form.Label>제목 : {communityNo}</Form.Label>
                    <Form.Control type="text" value={communityTitle.communityTitle} onChange={changeValue} name="communityTitle" />
